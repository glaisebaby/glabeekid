            ...
            <>
              <div className="relative flex h-full">
                <Popover.Button
                  ...
                  className="relative inline-flex items-center rounded-full border border-black/12 bg-white px-5 py-2.5 text-sm font-semibold text-black shadow-[0_10px_30px_rgba(15,23,42,0.08)] transition-all duration-200 ease-out hover:-translate-y-0.5 hover:border-black/25 hover:bg-black hover:text-white"
                >
                  Menu
                </Popover.Button>
              </div>
              ...
              >
                <PopoverPanel className="fixed inset-y-0 left-0 z-[60] flex w-full max-w-[420px] min-w-min flex-col p-4 text-sm text-ui-fg-on-color sm:p-5">
                  <div
                    ...
                    className="flex h-full flex-col justify-between overflow-hidden rounded-[34px] border border-black/8 bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(245,247,250,0.98))] p-6 text-black shadow-[0_32px_90px_rgba(15,23,42,0.18)]"
                  >
                    ...
                    <div className="relative flex justify-between">
                      ...
                      <button
                        ...
                        onClick={close}
                        className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-black/10 bg-white text-black/72 transition-colors hover:bg-black hover:text-white"
                      >
                        <XMark />
                      </button>
                    </div>
                    <ul className="relative mt-10 flex flex-col items-start justify-start gap-3">
                      {Object.entries(SideMenuItems).map(([name, href]) => (
                        <li key={name}>
                          <LocalizedClientLink
                            href={href}
                            className="group inline-flex items-center gap-3 rounded-full px-4 py-3 text-[2rem] font-semibold leading-none tracking-[-0.03em] text-black transition-all duration-200 hover:bg-black hover:text-white"
                            onClick={close}
                            data-testid={`${name.toLowerCase()}-link`}
                          >
                            <span className="h-2 w-2 rounded-full bg-black/18 transition-colors group-hover:bg-white/70" />
                            {name}
                          </LocalizedClientLink>
                        </li>
                      ))}
                    ...
                    <div className="relative mt-10 flex flex-col gap-y-5">
                      <div className="rounded-[28px] border border-black/10 bg-black px-5 py-5 text-white">
                        <Text className="text-[11px] font-semibold uppercase tracking-[0.28em] text-white/52">
                          Shopping notes
                        </Text>
                        <Text className="mt-3 text-sm leading-7 text-white/78">
